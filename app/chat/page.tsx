...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (query.trim().length >= 2) {
      const filtered = mockUserSearchResults.filter(user =>
  ...
  if (isMobileView) {
    return (
      <div className="flex flex-col min-h-screen bg-white">
        <NavBar 
          title={showList ? 'Chats' : (activeConv?.otherParticipant?.name || directChatOtherUser?.name || 'Chat')}
        ...
        {showList ? (
          <div className="flex-1 overflow-y-auto bg-white">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {showUserResults && (
                <div className="p-4">
                  <p className="text-sm font-semibold text-gray-900 mb-2 px-2">Start New Chat</p>
                  {searchingUsers ? (
                      ...
                      {userSearchResults.map(user => (
                        ...
                        >
                          <div className="flex-1">
                            ...
                            {user.rating > 0 && (
                              <p className="text-xs" style={{ color: PRIMARY_YELLOW }}>
                      ...
                      {filteredConversations.map(c => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between p-4 border-b border-gray-100 ${
                            activeId === c.chatId ? 'bg-gray-50' : 'bg-white'
                          }`}
                        >
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      <NavBar title={activeId ? (activeConv?.otherParticipant?.name || directChatOtherUser?.name || 'Chat') : 'Chat'} />
                    ...
                    {filteredConversations.map(c => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-4 border-b border-gray-100 hover:bg-gray-50 ${
                          c.chatId === activeId ? 'bg-gray-50' : 'bg-white'
                        }`}
                      >
                        <div className="flex-1 text-left">
                          <p className="text-sm font-semibold text-gray-900">
