...
const TrustCard: React.FC<TrustCardProps> = ({ feature }) => (
  <div className="flex items-start gap-4 p-3 md:p-5 rounded-xl bg-white border border-secondary-100 hover:border-secondary-200 hover:shadow-md transition-all">
    <div className={cn("size-10 md:size-12 rounded-md md:rounded-xl flex items-center justify-center shrink-0", feature.bgColor)}>
      <feature.icon className={cn("size-5 md:size-6", feature.color)} />
    </div>
    <div>
      <h3 className="font-bold text-secondary-900 md:text-lg mb-1">{feature.title}</h3>
      <p className="text-secondary-600 text-xs md:text-sm leading-relaxed">{feature.description}</p>
    </div>
  </div>
);
...
export const TrustSection: React.FC = () => {
  return (
    <section id="trust" className="py-12 md:py-20 bg-white">
