...
/**
 * Profile Edit Form
 * Clean, professional form for updating profile information
 */
...
export function ProfileEditForm({ user, onSave, onCancel }: ProfileEditFormProps) {
  ...
  const addSkill = () => {
    if (newSkill.trim() && !formData.skills.includes(newSkill.trim().toLowerCase())) {
      updateField("skills", [...formData.skills, newSkill.trim().toLowerCase()]);
      setNewSkill("");
    }
  };
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {formData.skills.map((skill) => (
            <Badge
              key={skill}
              variant="secondary"
              className="bg-gray-100 text-gray-700 hover:bg-gray-100 pr-1.5"
            >
              {skill}
              <button
