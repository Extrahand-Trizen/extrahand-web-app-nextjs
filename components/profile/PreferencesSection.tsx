  ...
  const toggleDayAvailability = (day: string) => {
    setLocalPrefs((prev) => {
      ...
      return {
        ...
        availability: {
          ...prev.availability,
          [dayKey]: currentDay ? undefined : [{ start: "09:00", end: "18:00" }],
        },
      };
    });
    setHasChanges(true);
  ...
  return (
    <div className="max-w-2xl space-y-4 sm:space-y-6">
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {TASK_CATEGORIES.map((category) => {
            const isSelected = localPrefs.preferredCategories.includes(category.id);
            return (
              <button
                key={category.id}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-7 gap-1 sm:gap-2">
            {DAYS_OF_WEEK.map((day) => {
              const dayKey = day.id as keyof typeof localPrefs.availability;
          ...
          <p className="text-[10px] sm:text-xs text-gray-500">
            Click on days to toggle availability. Default hours: 9 AM - 6 PM
          </p>
        </div>
      </CollapsibleSection>
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs sm:text-sm text-gray-600">50 km</span>
          </div>
          <Slider
      ...
      >
        <div className="space-y-4">
          <div>
            <Label className="text-xs sm:text-sm text-gray-700">Language</Label>
            <div className="grid grid-cols-2 gap-2 mt-2">
              {[
                ...
                <button
                  key={lang.id}
                  onClick={() => {
                    setLocalPrefs((prev) => ({ ...prev, language: lang.id }));
                    setHasChanges(true);
                  }}
                  className={cn(
          ...
          <div>
            <Label className="text-xs sm:text-sm text-gray-700">Currency</Label>
...
function CollapsibleSection({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {badge && (
              <Badge variant="secondary" className="text-[10px] sm:text-xs bg-gray-100 text-gray-600 flex-shrink-0">
                {badge}
              </Badge>
            )}
          </div>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 sm:px-5 sm:pb-5 border-t border-gray-100 pt-4">{children}</div>
      )}
    </div>
  );
}
