...
export function TaskOffersSection({
   ...
   const handlePaymentSuccess = async () => {
      ...
      toast.success("Payment successful!", {
         description: "Task assigned to helper.",
      });
                           ...
                           <div className="space-y-1">
                              {counterApplication.negotiation.history.map((entry, idx) => (
                                 <p key={idx} className="text-xs text-secondary-500">
                                    • {entry.action === 'counter' ? 'Counter' : 'Accepted'} by {entry.by === 'poster' ? 'Client' : 'Helper'}: {entry.action === 'counter' ? `₹${entry.amount?.toLocaleString() || 'N/A'}` : 'N/A'}
                                 </p>
                              ))}
                           </div>
                        </div>
